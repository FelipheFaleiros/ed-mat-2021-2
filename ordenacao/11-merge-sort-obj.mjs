...
function mergeSort (vetor, fnComp){
    ...
    // slice() divide um vetor da primeira posição informa
    // até a última posição informada (exclusive)
    let vetEsq = vetor.slice(0, meio)

    // Quando o segundo parâmetro de slice() é omitido, a 
    // fatia vai da posição informada até o final do vetor
    ...
    // Mesclagem ordenada de verEsq com vetDir 
    ...
    //Determina qual o vetores tem sobra
    let sobra 

    // Sobra à esquerda
    if (posEsq < posDir){
        // Copia para a sobra todos os elementos a partir
        //de posEsq até o fim de vetEsq
        sobra = vetEsq.slice(posEsq)
    }
    // Sobra à direita 
    else{
        // Copia para a sobra todos os elementos a partir 
        //de posDir até o fim de vetDir
        sobra = vetDir.slice(posDir)
    }
    juncoes++

    // O vetor final ordena será a concatenação de vetRes + sobra
    return[...vetRes,...sobra]
}
